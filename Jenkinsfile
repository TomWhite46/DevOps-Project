        ...
        stages{
            stage('Build'){
                steps{
                    ...
                    sh "docker login -u tomrwhite -p Miltiades!490 && cd ./backend && docker build -t docker.io/tomrwhite/backend-image:latest . && docker push docker.io/tomrwhite/backend-image:latest"
                }
            }
            stage('Testing'){
                steps{
